...
def how_much_to_send():
    mu = random.choice(MSAT_AMOUNTS_TO_SEND) / 100
...
def run_simulation(network, use_gp_protocol):
    counter = 0
...
def create_network(soft_griefing_percentage, griefing_percentage, delta, max_number_of_block_to_respond):
    network = Network()
...
def generate_redundancy_network(soft_griefing_percentage, griefing_percentage, delta, max_number_of_block_to_respond):
    # connect 2 nodes if differ by 10 to the power of n(1...floor(log_10(number_of_nodes))+1)
    #   modulo 10^floor(log_10(number_of_nodes))
    network = create_network(soft_griefing_percentage, griefing_percentage, delta, max_number_of_block_to_respond)
...
def generate_network_randomly(soft_griefing_percentage, griefing_percentage, channel_per_node, delta,
                              max_number_of_block_to_respond):
    network = create_network(soft_griefing_percentage, griefing_percentage, delta,
                             max_number_of_block_to_respond)
    for node in network.nodes:
...
@simulation_details
def simulate_random_network(soft_griefing_percentage=0.05, griefing_percentage=0.05, channel_per_node=10,
                            use_gp_protocol=True, delta=40, max_number_of_block_to_respond=5):
    network = generate_network_randomly(soft_griefing_percentage, griefing_percentage, channel_per_node, delta,
                                        max_number_of_block_to_respond)
    return run_simulation(network, use_gp_protocol)


def run_multiple_simulation(is_soft_griefing=True):
    # soft_griefing_percentages = [0, 0.05, 0.15]
    soft_griefing_percentages = [0.2]
    ...
    # use_gp_protocol_options = [False, True]
    network_topologies = ['redundancy'] # 'snapshot',
    # deltas = [40, 100]
    deltas = [40]
    # max_numbers_of_block_to_respond = [2, 8]
    max_numbers_of_block_to_respond = [1]
    try:
