...
def main():
    ...
    all_parameters_with_multiple = {k: v for k, v in all_parameters.items() if len(v) > 1}
    for i in range(1, len(all_parameters_with_multiple.keys()) + 1):
        for per in itertools.permutations(all_parameters_with_multiple.keys(), i):
            ...
            for values in itertools.product(*permutation):
                ...
                count = 0
                key = '_'.join([str(z[0]) if len(z) == 1 else 'Average' for z in values])
                for opt in itertools.product(*values):
                    for m in runs['_'.join([str(z) for z in opt])]:
                        metrics[m] += runs['_'.join([str(z) for z in opt])][m]
                    count += 1
                runs[key] = {k: v / count for k, v in metrics.items()}
    for r in runs:
        print(r, runs[r])
